...
from piot_ui_msg.msg import RobotStatus
...
class MyFrame(rtmgr.MyFrame):
	...
	def __init__(self, *args, **kwds):
		...
		#
		rclpy.init()
		self.node = rclpy.create_node('piot_ui')
...
#		self.pub = self.node.create_publisher(std_msgs.msg.String , 'from_rtmgr' , 10 )
		...
		self.sub_goal = self.node.create_subscription(Path, 'plan', self.robot_status_data_goal,10)
...
#			self.label_robot_password.SetValue(self.load_dic.get('connection_data').get('robot_password'))
		...
		bm = scaled_bitmap(wx.Bitmap(rtmgr_src_dir() + 'images/piot_logo_1.png'), 0.2)
		...
		# icon
		bm = scaled_bitmap(wx.Bitmap(rtmgr_src_dir() + 'images/piot_logo_2.png'), 0.5)
		icon = wx.EmptyIcon()
...
#		robot_password = self.label_robot_password.GetValue()
		...
		if v == True:
			self.robot_connect_proc = psutil.Popen("ros2 launch piot_bringup robot_launch.py", stdin=subprocess.PIPE, stdout=out, stderr=err, shell=True)
		elif v == False:
			self.robot_connect_proc = psutil.Popen("killall -9 robot_state_publisher piot_can_control_node piot_converter ekf_node rplidar_scan_publisher witmotion_ros_node itops_f07_node piot_battery_check_node", stdin=subprocess.PIPE, stdout=out, stderr=err, shell=True)
	...
	def OnSlam(self, event):
		...
		if v == True:
			...
			self.robot_slam_proc = psutil.Popen("ros2 launch piot_cartographer cartographer_launch.py",stdin=subprocess.PIPE, stdout=out, stderr=err, shell=True)

		elif v == False:
			self.button_save_map.Disable()
			self.robot_slam_proc = psutil.Popen("killall -9 rviz2 cartographer_node cartographer_occupancy_grid_node",stdin=subprocess.PIPE, stdout=out, stderr=err, shell=True)
	...
	def OnNavigation(self, event):
		...
		if v == True:
			self.robot_nav_proc = psutil.Popen("ros2 launch piot_navigation2 navigation2_launch.py map:=" + self.path_navigation ,stdin=subprocess.PIPE, stdout=out, stderr=err, shell=True)
		elif v == False:
	...
	def OnCameraConnect(self,event):
		...
		if v == True:
			self.robot_camera_connect_proc = psutil.Popen(['sshpass', '-p', robot_password, 'ssh', robot_id + '@' + robot_ip, '-oStrictHostKeyChecking=no', 'source ~/ros_colcon_ws/install/setup.bash && source ~/colcon_ws/install/local_setup.bash && ros2 launch stella_camera stella_camera.launch.py'], stdout=out, stderr=err)
		elif v == False:
	...
	def OnCameraShow(self,event):
		...
		if v == True:
			self.robot_cam_proc = psutil.Popen("gnome-terminal --tab -- ros2 run rqt_image_view rqt_image_view /camera && sleep 2 && wmctrl -r 'rqt_image_view__ImageView - rqt' -e 0,70,575,630,375",stdin=subprocess.PIPE, stdout=out, stderr=err, shell=True)
			
		elif v == False:
			self.robot_cam_proc = psutil.Popen("killall -9 python3",stdin=subprocess.PIPE, stdout=out, stderr=err, shell=True)

	def OnRTH(self,event):
		v = (event.GetEventObject()).GetValue()
		pub_patrol = self.node.create_publisher(Bool, 'patrol_flag',10)
		pub_rth = self.node.create_publisher(Bool,'rth_flag',10)   

		if v == True:
			rth_flag = Bool()
			rth_flag.data = True
			pub_rth.publish(rth_flag)
			patrol_flag = Bool()
			patrol_flag.data = False
			pub_patrol.publish(patrol_flag)
			set_val(self.button_patrol, False)
	...
	def OnPatrol(self,event):
		...
		if v == True:
			patrol_flag = Bool()
			patrol_flag.data = True
			pub_patrol.publish(patrol_flag)
			rth_flag = Bool()
			rth_flag.data = False
			pub_rth.publish(rth_flag)
			set_val(self.button_rth, False)
	...
	def OnPatrolSetting(self,event):
		v = (event.GetEventObject()).GetValue()
		out = subprocess.PIPE
		err = subprocess.STDOUT

		if v == True:
			self.robot_nav_proc = psutil.Popen("ros2 launch piot_patrol patrol_launch.py", stdin=subprocess.PIPE, stdout=out, stderr=err, shell=True)
		elif v == False:
			self.robot_nav_proc = psutil.Popen("killall -9 piot_patrol",stdin=subprocess.PIPE, stdout=out, stderr=err, shell=True)      
	...
	def robot_status_data_goal(self,data):
		self.robot_goal_x = data.poses[len(data.poses) -1].pose.position.x
		self.robot_goal_y = data.poses[len(data.poses) -1].pose.position.y
