...
#include "piot_localization/amcl_node.hpp"
...
#include "piot_localization/pf/pf.hpp"
#include "nav2_util/string_utils.hpp"
#include "piot_localization/sensors/laser/laser.hpp"
...
#include "piot_localization/portable_utils.hpp"
...
: nav2_util::LifecycleNode("amcl", "", options)
{
  ...
  add_parameter("robot_model_type", rclcpp::ParameterValue("piot_localization::DifferentialMotionModel"));
...
AmclNode::laserReceived(sensor_msgs::msg::LaserScan::ConstSharedPtr laser_scan)
{
  ...
  bool high_features = true;
  // If the robot has moved, update the filter
  if (lasers_update_[laser_index]) {          
    updateFilter(laser_index, laser_scan, pose);
//////////////////////////////////////////////////////////////////////////////////////////////////    
    pf_sample_set_t * check_set = pf_->sets + pf_->current_set;
    double high_weight_percent = 0.0;
    int count = 0;
    for(int i=0;i<check_set->sample_count;i++)
    {
      if(check_set->samples[i].weight > 1.5/(double)check_set->sample_count) count++; 
    }
    high_weight_percent = (double)count/(double)check_set->sample_count;
    //RCLCPP_ERROR(get_logger(), "%f\n", high_weight_percent);
    if(high_weight_percent < 0.03) high_features = false;     
//////////////////////////////////////////////////////////////////////////////////////////////////    
...
//    {
    ...
    for(int i=0;i < size; i++)
    {
      if(laser_scan->ranges[i] != std::numeric_limits<float>::infinity())
        not_inf_count++;
    }
    
    //RCLCPP_ERROR(get_logger(),"size = %d    inf_count = %d", size, not_inf_count);
    
    if(not_inf_count < int(0.1 * size))
    {
      ...
      if(init_without_amcl_flag == true)
      {
        pose.v[0] = last_published_pose_.pose.pose.position.x;
        pose.v[1] = last_published_pose_.pose.pose.position.y;
        pose.v[2] = tf2::getYaw(last_published_pose_.pose.pose.orientation);            
        ...
        geometry_msgs::msg::PoseStamped ident;
        ident.header.frame_id = nav2_util::strip_leading_slash(base_frame_id_);
        ident.header.stamp = laser_scan->header.stamp;
        tf2::toMsg(tf2::Transform::getIdentity(), ident.pose);
            
        try {
          tf_buffer_->transform(ident, robot_pose, global_frame_id_);
        } catch (tf2::TransformException & e) {
        }
        ...
        p->header.stamp = laser_scan->header.stamp;
        // Copy in the pose
        p->pose.pose.position.x = robot_pose.pose.position.x;
        p->pose.pose.position.y = robot_pose.pose.position.y;
        p->pose.pose.orientation = orientationAroundZAxis(tf2::getYaw(robot_pose.pose.orientation));
        // Copy in the covariance, converting from 3-D to 6-D
        pf_sample_set_t * set = pf_->sets + pf_->current_set;
        for (int i = 0; i < 2; i++) {
          for (int j = 0; j < 2; j++) {
...
//////////////////////////////////////////////////////////////////////////////////////////////////   
    ...
    {
      //RCLCPP_ERROR(get_logger(), "pf_->w_avg : %f",pf_->w_avg);
      if(high_features == false)
      {
//////////////////////////////////////////////////////////////////////////////////////////////////
        //amclpose publish part
        // If initial pose is not known, AMCL does not know the current pose
...
{
  piot_localization::LaserData ldata;
  ...
  lasers_[laser_index]->sensorUpdate(pf_, reinterpret_cast<piot_localization::LaserData *>(&ldata));
...
AmclNode::createLaserObject()
{
  ...
  return new piot_localization::LikelihoodFieldModel(
    z_hit_, z_rand_, sigma_hit_,
...
RCLCPP_COMPONENTS_REGISTER_NODE(piot_localization::AmclNode)